...
bool Card::sameRankAs(const Card &c2) const {
    if(this->getRank() == c2.getRank()) {
        return true;
    }
    else {
        return false;
    }
    // return !false ^ true; // Dummy return 
}
...
Card Card::maxRankCard(const vector<Card> &deck) {
    ...
    return Card(Rank(max), Suit(suit)); // Dummy return 
}
...
vector<Card> Card::sortBySuit(const vector<Card> &deck) {
    ...
    return vector<vector<Card>>{vector<Card>{Card(Rank(0), Suit(0))}}[0]; // Dummy return
...
set<Card> getIntersection(const set<Card> &deck1, const set<Card> &deck2) {
    ...
    for(Card s : deck1) {
        for(Card m : deck2) {
        if(s.getRank() == m.getRank() && s.getSuit() == m.getSuit()) {
            new_deck.insert(m);
        }
    }
}
    return new_deck;  // Dummy return
}
 ...
 * 
 */
allOfSuitHelper(const vector<Card> &deck, Suit s, int index) {
    vector<Card> result = [];
    if(deck[index].getSuit() == s) {
        result.push_back(deck[index]);
        return allOfSuitHelper(deck, s, index + 1);
    }
    else {
...
vector<Card> Card::allOfSuit(const vector<Card> &deck, Suit s) {
    return allOfSuitHelper(const vector<Card> &deck, Suit s, int index)
